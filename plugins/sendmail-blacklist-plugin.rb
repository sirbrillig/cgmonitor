...
class SendmailBlacklistPlugin < CGMonitor::Plugin
  def start_up
    @blacklists_seen = Hash.new
    @messages = Array.new
    my_options[:max_blacklists_per_domain] ||= 2
    my_options[:keep_blacklists_for] ||= 6.hours
    my_options[:time_between_warnings] ||= 30.minutes
    info_message("I will look for #{my_options[:max_blacklists_per_domain]} blacklists per domain within #{my_options[:keep_blacklists_for] / 60} minutes (#{my_options[:keep_blacklists_for]} seconds) and report every #{my_options[:time_between_warnings] / 60} minutes (#{my_options[:time_between_warnings]} seconds).")
  end
  ...
  def every_line(line)
    return unless line =~ /, relay=(\S+) .+?, dsn=[45]\..+?, stat=.*?(blacklist|blocked|blocklist)/i
    domain = $1;
